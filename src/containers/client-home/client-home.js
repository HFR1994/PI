...
class ClientHome extends Component {
    ...
    cargarUsuarios() {
        this.apiService.get({
            ...
            if (response.status === 200) {
                const ac = response.payload.accepted;
    ...
    mapearUsuarios() {
        for (var usr in this.state.usuariosRFC) {
            this.apiService.get({
                url: "/client/" + this.state.usuariosRFC[usr]['client'],
            }).then(response => {
                if (response.status === 200) {
                    for (var x in this.state.accepted) {
                        if (this.state.accepted[x]['client'] === response.payload['rfc']) {
                            response.payload['permiso'] = "Aprobado"
                            break
                        }
                    }
                    for (var y in this.state.waiting) {
                        if (this.state.waiting[y]['client'] === response.payload['rfc']) {
                            response.payload['permiso'] = "Esperando"
                            break
                        }
                    }
                    const aux = this.state.usuarios.slice()
                    aux.push(response.payload)
                    this.setState({
                        usuarios: aux
                    })
    ...
    calularCantidadEdad() {
        ...
        for (var x in this.state.usuarios) {
            const edad = parseInt(this.state.usuarios[x]['edad'], 10);
            if (edad < 20) {
                uno += 1
            }
            else if (edad < 30) {
    ...
    calcularDatos() {
        ...
        for (var usr in this.state.usuariosRFC) {
            this.apiService.get({
                url: "/company/" + this.props.usr + "/client/" + this.state.usuariosRFC[usr]['client'],
            }).then(response => {
                if (response.payload.terceros[this.props.usr]!== undefined){
                    if (datos[response.payload.terceros[this.props.usr]['producto']] === undefined){
                        datos[response.payload.terceros[this.props.usr]['producto']] = 1
                    }else{
                        let aux = datos[response.payload.terceros[this.props.usr]['producto']]
                        aux+=1
                        datos[response.payload.terceros[this.props.usr]['producto']] = aux
                    }
                }
                this.setState({
                    datos: datos
